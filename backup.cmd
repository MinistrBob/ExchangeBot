...
"%SZIP%" a -t7z -r -mx9 -mtc=on -mta=on -mtr=on -xr@exclude.txt -xr!__pycache__ -xr!pgdata "%BP%\%date_name%_%PNAME%.7z" "%BASEPATH%\%PNAME%\*"
"%SZIP%" a -t7z -r -mx9 -mtc=on -mta=on -mtr=on -p%mypass% -ir@exclude.txt "%BP%\%date_name%_%PNAME%_PASS.7z" "%BASEPATH%\%PNAME%\exclude.txt"
rem Delete backups except last 10
cd /d "%BP%"
for /f "skip=20 eol=: delims=" %%F in ('dir /b /o-d *.7z') do @del "%%F"
